...
Listed here are the output JSON objects of the API. so this is what the front-end will see. Fields are sorted by their necessity for various requests, unless stated otherwise in the route doc. Mandatory fields MUST be provided for POST requests in the request body. Optional fields can be absent from POST requests in the request body. If so, they will be null. Database fields are fields generated by the database and provided to the user for extram information, and need never be submitted.
